...
export default function AlertsTable(props: { tableData: RowObj[] }) {
    ...
    const handleAccept = () => {
        if (!editRowDataRef.current.driver || !editRowDataRef.current.serie || 
            !editRowDataRef.current.state || !editRowDataRef.current.truck) {
    ...
    const columns = [
        ...
        cell: (info) => {
            ...
            <Select
                defaultValue={editRowDataRef.current?.truck || ''}
        ...
        cell: (info) => {
            ...
            return rowIndex === editRowIndex ? (
            <Select
                defaultValue={editRowDataRef.current?.state || ''}
            ...
            <Badge
                colorScheme={info.getValue() === 'Inactivo' ? 'red' : 'green'}
                fontSize="md"
                fontWeight="500"
                >
                {info.getValue()}
